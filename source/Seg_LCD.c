...
void SegLCD_Set(uint8_t Value,uint8_t Digit){//Sets a value from 0-F to a specified Digit, with 1 being the leftmost, 4 being the rightmost.  Will not display error is Value is outside of 0-F, but display will not update
	...
	if(Digit > 4){
		...
		else if(Value==0xFE){LCD->WF8B[LCD_Frontplane_Pin[((2*Digit)-2)]] = (LCD_CLEAR); LCD->WF8B[LCD_Frontplane_Pin[((2*Digit)-1)]] = LCD_CLEAR;}
	}
}//End SegLCD_Set
...
void printNumberLeft(uint8_t Value){
	if(Value>9){
		SegLCD_DisplayError(0x0F);
	}
	else {
		SegLCD_Set(254,4);
		SegLCD_Set(254,3);
...
void printNumberRight(uint8_t Value){
	if(Value>9){
		SegLCD_DisplayError(0x0F);
	}
	else {
...
void printNumber(uint16_t Value){
	if(Value>9999) {
		SegLCD_DisplayError(0x10); // out of range err
		return;
	}
	if(Value<10){
		SegLCD_Set(254,1);
...
void enableBlinkMode(){
	LCD->AR |= (1u<<LCD_AR_BLINK_SHIFT);
}

void disableBlinkMode(){
	LCD->AR &= ~(1u<<LCD_AR_BLINK_SHIFT);
}